...
/
alter table F15E4hist add created date ; 
alter table F15E4hist add created_by VARCHAR2 (255) ; 
alter table F15E4hist add row_version_number integer ; 
alter table F15E4hist add updated date ; 
alter table F15E4hist add updated_by VARCHAR2 (255) ; 
/
create or replace trigger F15E4hist_AUD_trig 
before insert or update on F15E4hist 
for each row 
begin 
  if inserting then 
    :new.created := localtimestamp; 
...
-- Table Relation_16 has a compound primary key so no sequence or trigger was created for it.
-- Table contacts has a compound primary key so no sequence or trigger was created for it.
DROP SEQUENCE role_code_seq ; 
create sequence role_code_seq 
start with 100 
increment by 1 
nomaxvalue 
;

create or replace trigger role_code_PK_trig 
before insert on role_code
for each row 
begin 
select role_code_seq.nextval into :new.role_id from dual; 
end; 
/
alter table role_code add created date ; 
alter table role_code add created_by VARCHAR2 (255) ; 
alter table role_code add row_version_number integer ; 
alter table role_code add updated date ; 
alter table role_code add updated_by VARCHAR2 (255) ; 
/
create or replace trigger role_code_AUD_trig 
before insert or update on role_code 
for each row 
begin 
  if inserting then 
    :new.created := localtimestamp; 
...
DROP SEQUENCE stat_code_seq ; 
create sequence stat_code_seq 
start with 100 
increment by 1 
nomaxvalue 
;

create or replace trigger stat_code_PK_trig 
before insert on stat_code
for each row 
begin 
select stat_code_seq.nextval into :new.code_id from dual; 
end; 
/
alter table stat_code add created date ; 
alter table stat_code add created_by VARCHAR2 (255) ; 
alter table stat_code add row_version_number integer ; 
alter table stat_code add updated date ; 
alter table stat_code add updated_by VARCHAR2 (255) ; 
/
create or replace trigger stat_code_AUD_trig 
before insert or update on stat_code 
for each row 
begin 
  if inserting then 
    :new.created := localtimestamp; 
...
CREATE INDEX F15E4Emp_emp_id_FK_0 ON Approvers(F15E4Emp_emp_id) ;
DROP INDEX F15E4RFE_RFE_id_FK_1 ;
CREATE INDEX F15E4RFE_RFE_id_FK_1 ON Approvers(F15E4RFE_RFE_id) ;
DROP INDEX F15E4Emp_emp_id_FK_2 ;
CREATE INDEX F15E4Emp_emp_id_FK_2 ON F15E4Auth(F15E4Emp_emp_id) ;
...
DROP INDEX stat_code_code_id_FK_11 ;
CREATE INDEX stat_code_code_id_FK_11 ON F15E4status(stat_code_code_id) ;
DROP INDEX F15E4RFE_RFE_id_FK_12 ;
CREATE INDEX F15E4RFE_RFE_id_FK_12 ON Relation_16(F15E4RFE_RFE_id) ;
DROP INDEX F15E4tasks_task_id_FK_13 ;
CREATE INDEX F15E4tasks_task_id_FK_13 ON Relation_16(F15E4tasks_task_id) ;
DROP INDEX F15E4Emp_emp_id_FK_14 ;
CREATE INDEX F15E4Emp_emp_id_FK_14 ON contacts(F15E4Emp_emp_id) ;
DROP INDEX F15E4RFE_RFE_id_FK_15 ;
CREATE INDEX F15E4RFE_RFE_id_FK_15 ON contacts(F15E4RFE_RFE_id) ;
DROP INDEX role_code_role_id_FK_16 ;
CREATE INDEX role_code_role_id_FK_16 ON contacts(role_code_role_id) ;
